...
namespace HMPlus.NPCs.TheIllusion {
	public class FigmentOfDarkness : ModNPC {
		...
		public override void SetStaticDefaults () {
			DisplayName.SetDefault ("Figment of Darkness");
		...
		void ShootAtPlayer () {
			if (npc.ai [0] < 100f) return;
			npc.ai [0] = 0f;
			if (Main.rand.Next (3) != 0) return;
			Vector2 velocity = (player.Center - npc.Center).SafeNormalize (new Vector2 (1f, 0f)) * 3.5f;
			Projectile.NewProjectile (npc.Center, velocity, mod.ProjectileType ("PhantasmaOrb"), 30, 6.0f, Main.myPlayer, 0, 1);
		}
	}
}