...
namespace HMPlus.NPCs.TheIllusion {
	public class FigmentOfHell : ModNPC {
		...
		public override void SetStaticDefaults () {
			DisplayName.SetDefault ("Figment of Hell");
		...
		void ShootAtPlayer () {
			if (npc.ai [0] < 90f) return;
			npc.ai [0] = 0f;
			if (Main.rand.Next (3) != 0) return;
			Vector2 velocity = (player.Center - npc.Center).SafeNormalize (new Vector2 (1f, 0f)) * 4.5f;
			Projectile.NewProjectile (npc.Center, velocity, mod.ProjectileType ("PhantasmaOrb"), 25, 6.0f, Main.myPlayer, 0, 2);
		}
	}
}